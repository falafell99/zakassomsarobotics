...
REAL_HEIGHTS = {
    "person": 1.70, "chair": 0.95, "couch": 0.85, "dining table": 0.75,
    "table": 0.75, "car": 1.50, "bus": 3.20, "truck": 2.80,
    "bicycle": 1.10, "motorcycle": 1.20, "dog": 0.55, "cat": 0.30,
    "bottle": 0.28, "backpack": 0.55, "suitcase": 0.70, "obstacle": 1.00,
}
...
if _ORT_OK and os.path.exists(_DEPTH_PATH) and os.path.getsize(_DEPTH_PATH) > 1_000_000:
    try:
        ...
        DEPTH_AI_OK = True
        print(f"[INIT] Depth Anything V2 loaded  ✅  (running at {DEPTH_SIZE}×{DEPTH_SIZE})")
...
def depth_navigate(depth_01: np.ndarray) -> tuple:
    ...
    def disp_to_m(d): return round(min(5.0, max(0.2, 0.5 / (d + 0.08))), 2)
...
def detect_hog(frame) -> list:
    ...
    boxes, weights = _hog.detectMultiScale(
        ...
        scale=1.05, hitThreshold=0.0, useMeanshiftGrouping=True,
    )
    ...
    for i, (x, y, bw, bh) in enumerate(boxes):
        ...
        if dm is not None:
            ...
            bx = np.clip(bx, 0, dm_w-1); by = np.clip(by, 0, dm_h-1)
        ...
        results.append({
            ...
            "box": (x1, y1, x2, y2), "distance": dist, "source": "HOG",
        })
    return results
...
# ══════════════════════════════════════════════════════════════════════════════
CMD_COLORS = {
    "CLEAR": (55, 215, 55), "STOP": (30, 30, 230), "DANGER": (0, 0, 255),
    "LEFT": (0, 215, 255), "RIGHT": (0, 215, 255), "IDLE": (140, 140, 140),
    "ERROR": (0, 0, 200),
}

def dist_color(d: float):
    if d < 0.4: return (0, 0, 255)
    if d < 0.8: return (0, 80, 255)
    if d < 1.5: return (0, 210, 255)
    if d < 3.0: return (55, 215, 55)
    return (110, 110, 110)
...
def draw_depth_overlay(frame, alpha=0.32):
    ...
    cv2.putText(frame, "DEPTH", (w - 72, 20),
...
def draw_zone_bars(frame, zones, h, w):
    ...
    for val, label, bx in [
        ...
        cv2.putText(frame, label, (bx + 4, by + 22),
...
def draw_direction_arrow(frame, cmd, w, h):
    ...
    if cmd == "LEFT":
        pts = np.array([[ax, ay-26],[ax-size, ay],[ax, ay+26],
                        [ax, ay+12],[ax+36, ay+12],[ax+36, ay-12],[ax, ay-12]], np.int32)
    else:
        pts = np.array([[ax, ay-26],[ax+size, ay],[ax, ay+26],
                        [ax, ay+12],[ax-36, ay+12],[ax-36, ay-12],[ax, ay-12]], np.int32)
    ov = frame.copy()
...
def draw_hud(frame, w, h, cmd, dist, obj, scene, ai_msg, status, fps, busy,
             cool, zones, hog_n, depth_ok):
    font = cv2.FONT_HERSHEY_SIMPLEX
...
state = dict(
    command="IDLE", obstacle="—", distance=5.0,
    ai_message="Starting…", scene="", status="INIT",
    fps=0.0, api_busy=False, last_api_time=0.0, force_api=False,
)
...
def startup_print(cap):
    ...
    print("═"*64)
    print(f"  Depth Anything V2: {'✅ loaded  (' + str(DEPTH_SIZE) + 'px)' if DEPTH_AI_OK else '❌ not found'}")
...
def main():
    ...
    while True:
        ...
        with _lock:
            ...
            if force: state["force_api"] = False
        ...
        if not busy and (force or cooldown):
            ...
            threading.Thread(target=call_api, args=(send, dist_hint), daemon=True).start()
        ...
        draw_hud(display, w, h, cmd, dist, obj, scn, ai_msg, status,
                 fps, busy, cool, depth_zones, len(hog_dets), DEPTH_AI_OK)
